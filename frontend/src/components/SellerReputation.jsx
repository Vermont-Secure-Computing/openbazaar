...
export default function SellerReputation({ merchantAuthority, allowInitialize = false }) {
    ...
    async function initialize() {
        try {
            ...
            setError(
                ...
                e?.message ||
                "Unable to initialize reputation."
            );
        } finally {
    ...
    if (loading) return <p className="seller-reputation-loading">Loading seller reputation...</p>;
        ...
        return (
            <section className="seller-reputation">
                ...
                {allowInitialize && (
                    <button
                        onClick={initialize}
                        disabled={initializing || !wallet.publicKey}
                    >
                        {initializing ? "Initializing..." : "Initialize Reputation"}
                    </button>
                )}
    ...
    const rows = [
        [5, reputation.fiveStar],
        [4, reputation.fourStar],
        [3, reputation.threeStar],
        [2, reputation.twoStar],
        [1, reputation.oneStar],
    ];
    ...
    return (
        <section className="seller-reputation">
            ...
            <div className="seller-rating-summary">
                <strong>{reputation.averageRating.toFixed(1)} ★</strong>
                <span>
                    {reputation.totalReviews} review
                    {reputation.totalReviews === 1 ? "" : "s"}
                </span>
            </div>

            <div className="rating-breakdown">
                {rows.map(([stars, count]) => {
                    const percent = reputation.totalReviews
